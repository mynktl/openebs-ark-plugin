...
func (s *serverUtils) SendSnapshot(bwriter *blob.Writer) error {
	var event syscall.EpollEvent
	var events [MaxEpollEvents]syscall.EpollEvent
	var buf [32 * 1024]byte

	s.Log.Infof("Uploading snapshot now")

	ipAddr := GetHostIp()
	s.Log.Infof("got ip %s", ipAddr)
	...
	for {
		...
		for ev := 0; ev < nevents; ev++ {
			if int(events[ev].Fd) == fd {
				...
				s.Log.Infof("3Downloading snapshot now fd: %v", connFd)
				...
				if events[ev].Events == syscall.EPOLLIN {
					for {
						...
						s.Log.Infof("4Downloading snapshot read nbytes: %v", nbytes, e)
						if nbytes > 0 {
							...
							s.Log.Errorf("got error with nbytes %v erR:%v", nbytes, e)
							if e == syscall.EAGAIN {
								break
							} else {
...
func (s *serverUtils) ReceiveSnapshot(breader *blob.Reader) error {
	var event syscall.EpollEvent
	var events [MaxEpollEvents]syscall.EpollEvent
	var buf [32 * 1024]byte

	s.Log.Infof("Downloading snapshot now")
